    ...
    it('uploads form correctly', async () => {
        ...
        expect(
            wrapper
                ...
                .at(0)
                .text(),
        ).toEqual('Form name is required');
        expect(
            wrapper
                .find('.text-danger')
                .at(1)
                .text(),
        ).toEqual('Form is required');

        wrapper
            .find('input[name="form_name"]')
            .simulate('change', { target: { name: 'form_name', value: 'test name' } });
        wrapper.find('input[name="module"]').simulate('change', { target: { name: 'module', value: 'test module' } });
        wrapper.find('input[name="form"]').simulate('change', { target: { name: 'form', files: sampleFile } });
        wrapper.find('Button').simulate('submit');
        await new Promise<any>(resolve => setImmediate(resolve));
        await flushPromises();
        wrapper.update();
