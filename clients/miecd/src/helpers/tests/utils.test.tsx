...
describe('helpers/utils', () => {
  ...
  it("Ensure groupBy works correctly", () => {
    expect(groupBy(smsDataFixtures.slice(0, 3), "event_id")).toEqual(
      groupedSmsData
    );
  });
});
...
describe("getModuleLink", () => {
  it("returns the correct value given specific input", () => {
    ...
    expect(getModuleLink(NBC_AND_PNC_CHILD)).toEqual(
      NBC_AND_PNC_COMPARTMENTS_URL
    );
    expect(getModuleLink(NBC_AND_PNC_WOMAN)).toEqual(
      NBC_AND_PNC_COMPARTMENTS_URL
    );
  });
});
...
describe("location data is available", () => {
  expect(
    locationDataIsAvailable(villages, communes, districts, provinces)
  ).toEqual(4);
